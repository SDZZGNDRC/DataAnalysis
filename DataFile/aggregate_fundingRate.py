...
def aggregate_funding_rates(paths: list[Path], output_path: Path) -> None:
    '''
    Aggregate funding rate data files. 
    
    Given a list of data files, this function will aggregate them into a single file.
    * The output json file's schema is defined in `schema/FundingRate.json`, and is the same as the schema of the input data files.
    ...
    for data_name in data_names:
        ...
        if data_name.category != "FundingRate":
            raise ValueError(f"Data name {data_name.name} must have the category 'FundingRate'")
    ...
    for path in paths:
        try:
            ...
            validate(instance=file_data, schema=get_schema("FundingRate"))
    ...
    try:
        validate(instance=merged_root, schema=get_schema("FundingRate"))

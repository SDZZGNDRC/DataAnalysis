...
class TV:
    ...
    def _calc(self, bookcore: BookCore) -> float:
        if self.side == 'ask':
            ...
            return sum(map(lambda x: x.price * x.amount, asks))
        elif self.side == 'bid':
            L = bookcore.depth_bids
            if L < self.N:
                raise ValueError(f"N must be smaller than max-depth, but {L} < {self.N}")
            bids = bookcore.bids[:self.N]
            return sum(map(lambda x: x.price * x.amount, bids))
        else:
            raise ValueError(f"side must be 'ask' or 'bid', but {self.side} was given.")
